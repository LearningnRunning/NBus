...
class ReserveDao:
    ...
    def insert(self, a:Reserve):
        ...
        # 3. 실행할 sql문 정의
        sql = 'insert into reserve(id, arrmsg, plainNo, stNm, stNmD, reserve, etc) ' \
              'values(%s, %s, %s, %s, %s, %s, %s)'

        # 4. sql 문에 %s를 사용했다면 각 자리에 들어갈 값을 튜플로 정의
        d = (a.id, a.arrmsg, a.plainNo, a.stNm, a.stNmD, a.reserve, a.etc)
    ...
    def select(self, id:str):
        try:
            ...
            if row:
                return Reserve(row[0],row[1],row[2],row[3],row[4],row[5],row[6])
    ...
    def selectByName(self, name:str): #name 기준 검색, 여러개 검색
        ...
        try:
            ...
            d = (name,)
            cursor.execute(sql, d)  # sql 실행
            res =[Reserve(row[0],row[1],row[2],row[3],row[4],row[5],row[6])
                  for row in cursor]
            # for row in cursor:
    ...
    def update(self, a:Reserve):
        try:
            self.connect()  # db연결
            cursor = self.conn.cursor()  # 사용할 커서 객체 생성
            sql = 'update reserve set arrmsg=%s, plainNo=%s, stNm=%s,' \
                  'stNmD=%s, reserve=%s, etc=%s' \
                  'where id = %s'

            d = (a.arrmsg, a.plainNo, a.stNm, a.stNmD, a.reserve, a.etc, a.id)
            cursor.execute(sql, d)  # sql 실행
            self.conn.commit()
