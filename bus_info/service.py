...
from bus_info.vo import Bus1, Bus2, Bus3, Bus4


class Service:
    def __init__(self):
        self.key = 'AKfan37NDGnLV%2FFaafUPpYaYwl2YG91sOrLFQX1vco5iqRInax4RzmyoEDOLGXIUoXlKmz0mCnEToQn7wRjtOg%3D%3D' # 정류소정보조회


    def getStationByName(self, stSrch): # 버스 이름으로 검색하는 메소드 생성
        ...
        for route in routeList:
            stId = route.find('stId').text
            arsId = route.find('arsId').text

            res.append(Bus1(stId=stId, arsId=arsId))
    ...
    def getRouteByStation(self, arsId): # getBusInfoByStationId
        ...
        for route in routeList:
            ...
            res.append(Bus2(busRouteId=busRouteId, busRouteNm=busRouteNm))

        return res

    #seq=None, name=None, direction=None, x=None, y=None
    def getStaionByRoute(self, busRouteId): # getStationInfoById
        ...
        for station in stationList:
            ...
            res.append(Bus3(busRouteId=busRouteId,seq=seq))
    ...
    def getArrInfoByRoute(self, stId,busRouteId, seq):
        ...
        for station in stationList:
            ...
            res.append(Bus4(stNm=stNm,rtNm=rtNm, firstTm=firstTm,lastTm=lastTm, term=term,
